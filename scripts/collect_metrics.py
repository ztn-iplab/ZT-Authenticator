...
def get_json(base_url: str, path: str) -> dict:
    with request.urlopen(f"{base_url}{path}", timeout=10) as resp:
        return json.loads(resp.read().decode("utf-8"))
...
def zt_verify(base_url: str, enrollment: Enrollment, otp: str, key: Ed25519PrivateKey) -> Tuple[bool, str, float]:
...
def totp_verify(base_url: str, enrollment: Enrollment, otp: str) -> Tuple[bool, str, float]:
    started = time.perf_counter()
    resp = post_json(
        base_url,
        "/totp/verify",
...
def login_recover(base_url: str, email: str, code: str) -> Tuple[bool, str, float]:
    started = time.perf_counter()
    resp = post_json(
        base_url,
        "/login/recover",
...
def rotate_key(base_url: str, enrollment: Enrollment, private_key: Ed25519PrivateKey) -> Tuple[bool, str, float]:
    started = time.perf_counter()
    public_key = private_key.public_key()
    public_bytes = public_key.public_bytes(
        encoding=serialization.Encoding.Raw,
...
def main() -> None:
    ...
    for _ in range(args.trials):
        ...
        ok, reason, latency = totp_verify(base_url, enrollment, otp)
        rows.append(("legitimate_login", "standard_totp", ok, reason, latency))

        ok, reason, latency = zt_verify(base_url, enrollment, otp, enrollment.private_key)
    ...
    for _ in range(args.trials):
        ...
        ok, reason, latency = zt_verify(base_url, enrollment, otp, attacker_key)
    ...
    for i in range(args.recovery_trials):
        ...
        trial_enroll = enroll(base_url, trial_email, trial_rp)
        otp = current_otp(trial_enroll.secret)
        ok, reason, latency = totp_verify(base_url, trial_enroll, otp)
        rows.append(("offline_degraded", "standard_totp", ok, reason, latency))

        code = trial_enroll.recovery_codes[0]
        ok, reason, latency = login_recover(base_url, trial_email, code)
    ...
    ok, reason, rotate_ms = rotate_key(base_url, enrollment, new_key)
    if ok:
        otp = current_otp(enrollment.secret)
        ok, reason, verify_ms = zt_verify(base_url, enrollment, otp, new_key)
